...
export function AddProductToDataBase(productParam: ProductType){
  ...
  var resultMessage = "";
  request.post("/product/addProduct",productParam).then((res:AxiosResponse<resultProductType>)=>{
    if(res.data !== undefined ){
      if(res.data.code===200){     
        // save product Name to local storage
        resultMessage = "productName:" + res.data.productInfo.productName +" is added successfully!";       
      }else{        
