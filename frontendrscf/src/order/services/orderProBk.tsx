...
interface Iprops{
  setShowSelectPro:Function
  setSelProductList:Function
  selProductList:any
}
interface shopType{
  id?:number
...
function SelectPro(props:Iprops) {
  // 搜索参数
  ...
  useEffect(()=>{
    //  用shopList更新右边的数据
    setTargetData(props.selProductList)
  },[])
  function toRight(){
  ...
  return ( <div className="SelectShop">
    ...
    <div className='footer'>
      <Button onClick={()=>props.setShowSelectPro(false)}>取消</Button>
      {/* 单击确定更新shopList */}
      <Button type='primary' onClick={()=>{
        // 右边的数据为选择的店铺
        props.setSelProductList(targetData);
        // 关闭弹框
        props.setShowSelectPro(false)
      }}>确定</Button>
    </div>
  </div> );
}
